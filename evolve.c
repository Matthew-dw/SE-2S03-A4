...
{ 
    double l = ((struct Individual *)p)->fitness; 
    double r = ((struct Individual *)q)->fitness;  
    return (l - r); 
} 
PPM_IMAGE *evolve_image(const PPM_IMAGE *image, int num_generations, int population_size, double rate){
    ...
    //Compute Fitness
    comp_fitness_population(image, population, population_size);
    //Sort population
    qsort(population, population_size, sizeof(*Individual), comparator);
